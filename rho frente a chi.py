...
# Importamos el localizador de marcas menores
from matplotlib.ticker import AutoMinorLocator

# ==========================================
# 1. CONFIGURACIÓN DE ESTILO (Idéntico a tus otros gráficos)
# ==========================================
mpl.rcParams.update({
    'font.size': 12,
...
# Paleta de colores académica y diferenciada (Mantiene tus colores exactos)
color_sin = '#4361ee'  # Azul cobalto para escenarios sin leptónicas (flag 0)
color_con = '#f76425'  # Tu color para escenarios con leptónicas (flag 1)
color_top = '#80ce68'  # Tu color verde para destacar el ajuste completo (flag 2)
...
# ==========================================
# 3. REPRESENTACIÓN GRÁFICA COMBINADA
# ==========================================

# A. Región sombreada ideal para el chi2/dof (Alrededor de 1)
...
# B. Banda de independencia estadística en torno a rho = 0
...
# C. Definición de las máscaras basadas en tu fichero (0, 1 y 2)
mask_sin = (con_leptonica == 0)
mask_con = (con_leptonica == 1)
mask_top = (con_leptonica == 2)

# Puntos SIN ecuación leptónica (Círculos azules - flag 0)
if np.any(mask_sin):
...
# Puntos CON ecuación leptónica parciales (Cuadrados naranjas - flag 1)
if np.any(mask_con):
...
# D. PUNTO DESTACADO DEFINITIVO (Estrella verde grande - flag 2)
if np.any(mask_top):
    x_top = chi2_dof[mask_top][0]
    y_top = rho[mask_top][0]
    
    # Dibujado de la estrella verde de forma directa y limpia
...
# ==========================================
# 4. ESTÉTICA, MARCAS MENORES, LEYENDA Y GUARDADO
# ==========================================

# Activación de marcas menores (Minor Ticks)
ax.xaxis.set_minor_locator(AutoMinorLocator(2))  
ax.yaxis.set_minor_locator(AutoMinorLocator(2))  
ax.tick_params(which='minor', length=4, color='lightgray')  
ax.tick_params(which='major', length=7)

# Mantenemos tus fuentes y etiquetas
...
# Leyenda elegante integrada con marco gris claro definido
ax.legend(frameon=True, facecolor='white', edgecolor='gray', fontsize=12, loc='upper left')

# Ajustar límites dinámicamente para que no se corten los puntos ni el marcador grande
