...
priors = {
    ...
    'vud': gv.gvar(0.9, 1.0),
    'vus': gv.gvar(0.2, 1.0),
    'epsilon_d': gv.gvar(0.0, 0.001),
    'epsilon_s': gv.gvar(-0.0037, 0.001)
}  


def modelo(p):
    return { 
        'superallowed': p['vud']*(1+p['epsilon_d']), 
        'neutron':      p['vud']*(1+p['epsilon_d']), 
        'semileptonic': p['vus']*(1+p['epsilon_s']), 
        #'T-inclusive':  p['vus']*(1-p['epsilon_s']), 
        'Leptonic':     (p['vus']/p['vud'])*(1-p['epsilon_s'])/(1-p['epsilon_d']), 
        'semi_2': (p['vus']/p['vud'])*(1+p['epsilon_s'])/(1+p['epsilon_d']),
        'T-exclusive':   (p['vus']/p['vud'])*(1-p['epsilon_s'])/(1-p['epsilon_d']),
        #'Rus': (p['vus']*(1-p['epsilon_s']))**2 * (Rud.mean / (p['vud']*(1-p['epsilon_d']))**2 - Delta.mean)
    }
...
print(f"Epsilon_d = {fit.p['epsilon_d'].mean:.5f} +/- {fit.p['epsilon_d'].sdev:.5f}")
print(f"Epsilon_s = {fit.p['epsilon_s'].mean:.5f} +/- {fit.p['epsilon_s'].sdev:.5f}\n")
